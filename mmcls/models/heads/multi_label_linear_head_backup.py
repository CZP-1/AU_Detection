...
class MultiLabelLinearClsHead(MultiLabelClsHead):
    ...
    def forward_train(self, x, gt_label, **kwargs):
        ...
        ## circle_loss with mixup
        neg_idx = torch.where(gt_label <= 0.5)
        pos_idx = torch.where(gt_label > 0.5)

        cls_score[pos_idx] = -cls_score[pos_idx]
        cls_score[neg_idx] = cls_score[neg_idx]
        tmp = cls_score.clone()
        
        cls_score[pos_idx] = cls_score[pos_idx] - 1e12
        tmp[neg_idx] = tmp[neg_idx] - 1e12
        zeros = torch.zeros_like(cls_score[..., :1])
        y_pred_neg = torch.cat([cls_score, zeros], axis=-1)
        y_pred_pos = torch.cat([tmp, zeros], axis=-1)
